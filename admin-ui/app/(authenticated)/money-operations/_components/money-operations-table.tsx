/**
 * Money-operations table (Epic 18). Lists proposed treasury moves and opens a
 * detail drawer on view — loading the full operation (incl. its review thread)
 * via a server action before opening. The progress column shows the N-eyes
 * rule ("1 of 2 approved").
 */
"use client";
...
import { loadMoneyOperationAction } from "@/app/(authenticated)/money-operations/_actions";
...
} from "./money-operation-detail-drawer";

export function MoneyOperationsTable({
  operations,
  tenantId,
  canApprove,
  currentAdminId,
  ...
  const [detail, setDetail] = React.useState<MoneyOperation | null>(null);
  const [open, setOpen] = React.useState(false);
  const [loadingId, setLoadingId] = React.useState<string | null>(null);

  const openDetail = async (id: string) => {
    setLoadingId(id);
    const result = await loadMoneyOperationAction(tenantId, id);
    setLoadingId(null);
    if (!result.ok) {
      toast({
        ...
        <Table>
          ...
          <TableBody>
            {operations.map((op) => (
              <TableRow key={op.id}>
                <TableCell>
                  <Badge variant="info">{moneyOperationLabel(op.operation)}</Badge>
                </TableCell>
                <TableCell className="max-w-[280px] truncate text-sm text-foreground">
                  {moneyOperationSummary(op)}
                </TableCell>
                <TableCell>
