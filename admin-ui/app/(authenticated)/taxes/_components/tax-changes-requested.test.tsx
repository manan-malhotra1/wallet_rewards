/**
 * Interaction tests for TaxChangesRequested — the "Open requests" section on
 * the Taxes page, from the admin's point of view.
 *
 ...
 * tax change. The revise dialog is stubbed to its trigger; server actions are
 * mocked.
 */
...
import { TaxChangesRequested } from "@/app/(authenticated)/taxes/_components/tax-changes-requested";
import { TooltipProvider } from "@/components/ui/tooltip";
import type { ConfigChangeRequest, Instrument } from "@/lib/api-types";

vi.mock("./create-tax-dialog", () => ({
  CreateTaxDialog: (props: { trigger?: React.ReactNode }) => <>{props.trigger}</>,
}));
...
describe("Tax change open requests", () => {
  it("Verify the maker can withdraw a pending tax change", async () => {
  ...
  it("Verify the maker can reopen a returned tax change to edit it", () => {
