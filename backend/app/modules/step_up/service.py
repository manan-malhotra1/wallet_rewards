...
async def enforce_step_up_for_user(
    session: AsyncSession,
    *,
    tenant_id: UUID,
    user_id: UUID,
    ...
    """Step-up variant for admin-on-behalf-of-user flows (treasury withdraw).

    Reuses the same step_up_policies threshold lookup + bcrypt verify as
    enforce_step_up but accepts an explicit user_id and an AdminPrincipal
    actor. The lockout counter still tracks the TARGET user (NFR-0190 —
    failed PINs lock the user regardless of who entered them), while the
    audit row is attributed to the admin who initiated the transaction.

    Args:
        tenant_id: Tenant of both admin and target user (caller must
            ensure they match).
        user_id: The user whose wallet is being moved.
        admin: Authenticated admin initiating the action.
        transaction_type: Matches the policy scope ('withdraw', 'fund').
        currency: Matches the policy scope.
        amount: The amount being moved.
        pin: Plain-text PIN re-submitted by the admin on behalf of the
            user. None on the first attempt — triggers StepUpRequired.

    Raises:
        StepUpRequired (401): policy says PIN needed but caller didn't send one.
        InvalidStepUpPin (401): PIN supplied but didn't verify.
        AppHTTPException (401, user_locked_out): user is locked out.

    Side effects:
        Writes a `pin.step_up_ok` or `pin.step_up_failed` audit row
        attributed to the admin. Does NOT commit — caller commits as
        part of the transaction it's protecting.
    """
    policy_q = await session.execute(
        select(StepUpPolicy).where(
    ...
    policy = policy_q.scalar_one_or_none()
    if policy is None:
        return  # No policy → no step-up.

    threshold = Decimal(str(policy.threshold_amount))
    if amount <= threshold:
        return  # Below threshold — admin session is sufficient.
    ...
    # Lockout is keyed on the target user — the admin is the actor, but
    # the PIN belongs to the user and so does the lockout state.
    ...
    pin_hash_q = await session.execute(
        select(User.pin_hash).where(User.id == user_id)
    )
    pin_hash = pin_hash_q.scalar_one_or_none()
    if not pin_hash or not hashing.verify_pin(pin, pin_hash):
