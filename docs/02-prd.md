...
| Pay-PRD-1295 | Per-transaction redemption caps | An Administrator must be able to cap a single internal redemption per currency: (a) an absolute maximum points per transaction, and/or (b) a maximum percentage of the user's current points balance per transaction (e.g. balance 100, 10% cap → at most 10 points per redemption). A redemption exceeding either configured cap is rejected before any ledger write (422 `redemption_txn_cap_exceeded`). Both caps are optional (NULL = uncapped on that axis) and are configured alongside the conversion rate via the same maker-checker flow — preventing a user's points from being drained in one transaction. | P0 |
