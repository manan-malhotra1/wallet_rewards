...
| D8 | Parent commission is a **% of the transaction amount** with the **same amount bands and precedence** as the child terms, so both legs ride one config row. Parent terms are **required** in the create request and may be zero | Symmetric with the child terms, one band-replace, one resolver, no new join. Required-not-defaulted because zero must be a decision, not an omission — a default would let a caller ship a rule whose parent leg silently pays nothing when they meant otherwise. This is only safe because migration 0069 backfills explicit zeros into every non-terminal commission config request: `config_requests/apply.py` re-validates a **stored** payload at **approval** time, so without that backfill every pending pre-deploy request would 422 forever (see §13 R7) |
...
| D19 | Every statement row **names its counterparty**, falling back from the other party's name → what the other account IS → the viewer's own other wallet | Raised against the commission rows, but the same blank cell had been reported for merchant cash-in. The old frontend fix was a per-transaction-type label map, so any type nobody remembered to add fell back to "—" — which is exactly how the commission types regressed. Deriving the label from the account on the other leg removes that maintenance trap; a new transaction type can never reintroduce the dash |
...
| R7 | Making the parent fields required strands commission config requests written before they existed — `apply.py` re-validates a stored payload at approval, so a pending request would 422 forever with no way for either the maker or the checker to clear it | Migration 0069 backfills explicit zeros into every **non-terminal** (PENDING / CHANGES_REQUESTED) commission payload, in both the `{"bands": [...]}` and flat shapes. APPLIED / WITHDRAWN rows and `config_request_revisions` snapshots are left untouched so the audit trail still shows what was actually proposed. Zero is the correct value because it reproduces the behaviour those requests were written under |
| R8 | Counterparty labels name operator infrastructure ("Bank mirror · Primary", "Cash float"), and `_build_recent_txns_payload` is shared by the admin statement **and** the mobile `/me/wallet` feed — so end users see them too | Accepted deliberately: the labels carry no PII and no other user's data, and a blank cell was worse. Flagged as a product decision — if operator naming should stay operator-only, the split belongs behind the existing admin-only flag on that function, not in the label map |
