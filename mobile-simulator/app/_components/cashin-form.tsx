/**
 * <CashInForm> — an agent funds a customer's wallet (cash-in), earning a
 * commission. The agent picks the customer + amount; the success message shows
 * the fee / commission / tax breakdown returned by the backend.
 *
 * Mirrors <P2PForm>'s inline step-up flow: if the amount is over the step-up
...
import { cashInAction } from "@/app/_actions";
import type { UserKey } from "@/lib/config";

// Customers an agent can cash-in for (the two consumer wallets). Labels are
// inlined so this client component doesn't import the server-only `config`.
const CUSTOMERS: { key: UserKey; label: string }[] = [
...
function SubmitButton() {
  ...
  return (
    ...
    >
      ...
      {pending ? "Cashing in…" : "Cash in"}
    </button>
  );
}

export function CashInForm({ agent }: { agent: UserKey }) {
  const [customer, setCustomer] = React.useState<UserKey>("alice");
  ...
  async function action() {
    const result = await cashInAction(agent, customer, amount, pin || undefined);
    if (result.ok) {
    ...
    <div className="mt-3 border-t border-[var(--color-border)] pt-3">
      ...
      <form action={action} className="flex flex-col gap-2">
        <div className="flex flex-wrap items-center gap-2">
          <select
            value={customer}
            onChange={(e) => setCustomer(e.target.value as UserKey)}
        ...
        {needsPin && (
          <div className="flex items-center gap-2 rounded-md border border-[var(--color-teal)] bg-[var(--color-teal)]/10 px-2 py-1.5">
            ...
            <span className="text-[10px] text-[var(--color-fg-muted)]">
              Amount above the step-up threshold — re-enter PIN and Cash in again.
            </span>
          </div>
        )}
      </form>
